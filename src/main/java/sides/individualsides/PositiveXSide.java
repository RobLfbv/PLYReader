...
/**
 * 
 * Classe qui fournit les methodes a un canvas qui est vu du cote x positif
 * (camera en x=infini)
 *
 */
public class PositiveXSide extends Side {
	...
	public List<Transformation> getTranslationFromDrag(double xDelta, double yDelta) {
		...
		if (xDelta != 0)
			transformationMatrices.add(new TransformationZTranslation(-xDelta));
	...
	public List<Transformation> getRotationFromDrag(double xDelta, double yDelta) {
		...
		if (yDelta != 0)
			transformationMatrices.add(new TransformationZRotation(-yDelta));
		return transformationMatrices;
	}
